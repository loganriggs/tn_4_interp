# %%
"""
Iterative pruning of the 2-layer memory model.
"""
...
NUM_LAYERS = 2
RANK = 11
SEED = 3  # Best seed from training

PRUNE_PERCENT = 0.10  # Prune 10% of remaining weights each iteration
MAX_ITERATIONS = 10
MAX_ACC_DROP = 0.02   # Stop if accuracy drops more than 2%
FINETUNE_STEPS = 3000

print(f"Device: {device}")
print(f"Iterative pruning of 2-layer memory model (seed {SEED})")
...
# LOAD MODEL
# =============================================================================
ckpt_path = checkpoint_dir / f"2layer_n10_memory_seed{SEED}.pkl"
...
for r in results:
    ...
    print(f"  {r['iteration']}: acc={r['acc_after_ft']:.1%} (Δ={-r['acc_drop']:+.1%}), sparsity={r['sparsity']:.1%}{marker}")
...
if best_within_threshold:
    ...
    # Save
    save_path = checkpoint_dir / f"2layer_n10_memory_seed{SEED}_sparse.pkl"
    ...
    print("\nNo iteration within 1% threshold!")
